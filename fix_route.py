path = r"G:\openclaw\DocMind\backend\app\routers\chat.py"
with open(path, "r", encoding="utf-8") as f:
    content = f.read()

marker = '    )\n\n\n@router.get("/conversations/{conv_id}"'
if marker not in content:
    marker = '    )\n\n\n\n@router.get("/conversations/{conv_id}"'
if marker not in content:
    print("MARKER NOT FOUND, trying fallback")
    idx = content.find('@router.get("/conversations/{conv_id}"')
else:
    idx = content.find(marker)

new_route = '''@router.post("/conversations", response_model=ConversationResponse)
async def create_conversation(
    body: dict,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    title = body.get("title", "New conversation")
    chat_type = body.get("chat_type", "general")
    document_id = body.get("document_id")

    conv = Conversation(
        id=str(uuid.uuid4()),
        title=title[:50] if len(title) > 50 else title,
        user_id=current_user.id,
        document_id=document_id,
        chat_type=chat_type
    )
    db.add(conv)
    await db.flush()

    return ConversationResponse(
        id=conv.id, title=conv.title, user_id=conv.user_id,
        chat_type=conv.chat_type, document_id=conv.document_id,
        messages=[],
        created_at=conv.created_at, updated_at=conv.updated_at
    )


'''

if idx >= 0:
    content = content[:idx] + new_route + content[idx:]
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(content)
    print(f"Route inserted at position {idx}")
else:
    print("Failed to find insertion point!")
